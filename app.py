...
MODEL_PATH = 'model/mango_effnet.pth'  # Changed to mango model
DISEASE_JSON_PATH = 'mango_disease_info.json'  # Changed to mango disease info
...
model.to(device)
model.eval()
print("🧠 Mango EfficientNet model loaded.")  # Updated print message
...
def send_daily_report():
    if daily_stats["count"] == 0:
        return
    try:
        msg = Message("📊 Daily Click Report - Mango Disease Detection",  # Updated email subject
...
def send_prediction_result_email(filename, prediction_results, image_path=None):
    try:
        msg = Message("🥭 New Mango Disease Detection Result",  # Updated email subject
                      sender=app.config['MAIL_USERNAME'],
                      recipients=['cropai2025@gmail.com'])
...
def send_email():
    ...
    if not (name and email and msgt):
        ...
        return redirect('/')
    try:
        m = Message("🥭 New Contact Request - Mango Disease Detection",  # Updated email subject
                    sender=app.config['MAIL_USERNAME'], 
        ...
        r.body = f"Hi {name},\nWe received your message regarding mango plant disease detection."
        mail.send(r)
